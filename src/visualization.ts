...
function upsertStep(view: RunVisualization, step: VisualStep): void {
...
function matches(step: VisualStep, filter: VisualizationFilter): boolean {
  ...
  if (filter.collapseNoise && step.severity === "info" && step.status === "done" && step.stage === "planning") return false;
  return true;
}
