...
export interface RunVisualization {
  ...
  sandboxHealth?: { total: number; sandboxed: number; direct: number; timedOut: number };
  steps: VisualStep[];
  edges: VisualEdge[];
}
...
export function createRunVisualization(): RunVisualization {
  return { steps: [], edges: [] };
}
...
function upsertStep(view: RunVisualization, step: Partial<VisualStep> & { id: string }): void {
  const now = Date.now();
  ...
  if (index >= 0) {
    ...
    view.steps.push({ createdAt: now, ...step } as VisualStep);
  }
  view.currentStepId = step.id;
}

function addEdge(view: RunVisualization, edge: VisualEdge): void {
  if (view.edges.some((item) => item.from === edge.from && item.to === edge.to && item.label === edge.label)) return;
  view.edges.push(edge);
}

function stepIdFromReplay(event: ReplayEvent): string {
  const step = "step" in event && event.step ? event.step.stepIndex : 0;
  return `step-${step}`;
}

function stepRole(event: AgentEvent): StepRole | undefined {
  switch (event.type) {
...
export function applyAgentEvent(view: RunVisualization, event: AgentEvent): RunVisualization {
  ...
  switch (event.type) {
    ...
    case "tool_call": {
      ...
      if (view.currentStepId && view.currentStepId !== id) addEdge(view, { from: view.currentStepId, to: id, label: "calls" });
      break;
    }
    case "tool_result": {
...
export function applyReplayEvent(view: RunVisualization, event: ReplayEvent): void {
  if (event.type === "checkpoint") {
    const id = `checkpoint-${event.payload.snapshotId}`;
    view.lastGoodCheckpointId = id;
    upsertStep(view, { id, label: `checkpoint: ${event.payload.reason}`, stage: "execution", status: "done", severity: "info", role: "checkpoint", output: `messages=${event.payload.messageCount}`, durationMs: (event as { durationMs?: number }).durationMs });
...
function matches(step: VisualStep, filter: VisualizationFilter): boolean {
  if (filter.stage && step.stage !== filter.stage) return false;
  if (filter.status && step.status !== filter.status) return false;
  if (filter.severity && step.severity !== filter.severity) return false;
  if (filter.role && step.role !== filter.role) return false;
  if (filter.tool && step.tool !== filter.tool) return false;
  if (filter.file && step.file !== filter.file) return false;
  if (filter.branch && step.branch !== filter.branch) return false;
  if (filter.collapseNoise && step.severity === "info" && step.status === "done" && (step.stage === "planning" || step.stage === "review")) return false;
  if (filter.sinceStep) {
    const stepNum = parseInt(step.id.replace(/[^0-9].*/, ""));
    if (stepNum < filter.sinceStep) return false;
  }
  if (filter.untilStep) {
    const stepNum = parseInt(step.id.replace(/[^0-9].*/, ""));
    if (stepNum > filter.untilStep) return false;
  }
  return true;
}

export function visibleSteps(view: RunVisualization, filter: VisualizationFilter = {}): VisualStep[] {
  return view.steps.filter((step) => matches(step, filter));
}

export function inspectStep(view: RunVisualization, stepId: string): VisualStep | null {
  return view.steps.find((s) => s.id === stepId) ?? null;
}

export function exportRunVisualization(view: RunVisualization, filter: VisualizationFilter = {}): string {
  return JSON.stringify({ ...view, steps: visibleSteps(view, filter) }, null, 2);
}

export function exportRunVisualizationGraphviz(view: RunVisualization, filter: VisualizationFilter = {}): string {
  const steps = visibleSteps(view, filter);
  const lines: string[] = ["digraph run {", "  rankdir=LR;", "  node [shape=box fontname=monospace];", "  edge [fontname=monospace];"];
  const statusColor: Record<StepStatus, string> = {
    queued: "#888888", running: "#1E90FF", blocked: "#FF4500",
    failed: "#DC143C", repaired: "#FFA500", done: "#32CD32",
  };
  const stageShape: Record<RunStage, string> = {
  ...
  for (const step of steps) {
    ...
    const extras: string[] = [];
    if (step.tool) extras.push(`tool=${step.tool}`);
    if (step.file) extras.push(`file=${step.file}`);
    if (step.severity) extras.push(`severity=${step.severity}`);
...
export function renderRunVisualization(view: RunVisualization, filter: VisualizationFilter = {}): string {
  const steps = visibleSteps(view, filter).slice(-12);
  const statusGlyph: Record<StepStatus, string> = {
    queued: "○", running: "◉", blocked: "■", failed: "✗", repaired: "◆", done: "✓",
  };
  const stageColor: Record<RunStage, string> = {
...
export function renderRunMapCompact(view: RunVisualization, maxWidth: number = 40): string[] {
  const steps = visibleSteps(view, { collapseNoise: true }).slice(-10);
  const statusGlyph: Record<StepStatus, string> = {
    queued: "○", running: "◉", blocked: "■", failed: "✗", repaired: "◆", done: "✓",
  };
  const stageAbbr: Record<RunStage, string> = {
    planning: "PLN", execution: "EXE", review: "REV", repair: "RPR", completion: "CMP",
  };
  const lines: string[] = [];
  for (const step of steps) {
