...
export function createConflictLog(): ConflictLog {
  return { conflicts: [] };
}

export function resolveConflict(
  conflict: Pick<Conflict, "parties" | "description" | "severity">,
  revisionCount: number,
  maxRevisions: number
): { resolution: ConflictResolution; justification: string } {
  ...
  if (partyA === "security_auditor" || partyB === "security_auditor") {
    if (conflict.severity === "blocking") {
      return {
        ...
        justification:
          "Security Auditor blocking rejection requires human approval to override. No agent may override a security rejection.",
      };
    }
    return {
      resolution: "escalate_coordinator",
      justification:
        "Security Auditor flagged a non-blocking issue. Coordinator must decide whether to proceed with conditions.",
    };
  }

  if (
    (partyA === "reviewer" && partyB === "implementer") ||
  ...
  if (partyA === "test_strategist" || partyB === "test_strategist") {
    return {
      resolution: "escalate_coordinator",
      justification:
        "Test Strategist and Implementer disagree on coverage. Coordinator must determine acceptable coverage threshold.",
    };
  }
...
export function recordConflict(
  log: ConflictLog,
  conflict: Omit<Conflict, "id" | "timestamp">
): Conflict {
...
export function canOverride(
  overridingRole: ConflictParty,
  overriddenRole: ConflictParty,
  justification: string
  ...
  if (overriddenRole === "architect" && overridingRole === "reviewer") {
    if (!justification || justification.length < 20) {
      return {
        allowed: false,
        reason:
          "Reviewer must provide explicit justification (min 20 chars) to override Architect plan.",
      };
    }
