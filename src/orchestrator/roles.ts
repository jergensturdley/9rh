...
export function classifyRisk(task: string, plan?: string): RiskLevel {
...
export function requiresSecurityAudit(risk: RiskLevel): boolean {
  return risk === "high" || risk === "critical";
}

export function requiresTestStrategy(task: string, plan?: string): boolean {
...
export function isTrivialEdit(task: string): boolean {
  ...
  return (
    /(?:fix typo|rename variable|update comment|add docstring|whitespace|formatting|lint fix)/.test(
      text
    ) &&
    task.length < 120 &&
