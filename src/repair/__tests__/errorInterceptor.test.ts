...
describe("withErrorInterception", () => {
  ...
  it("propagates FATAL errors without repair", async () => {
    await expect(
      withErrorInterception(async () => { throw new Error("Fatal: invariant violated"); }, {
  ...
  it("re-runs fn after successful repair", async () => {
    ...
    const fn = async () => {
      attempts++;
      if (attempts === 1) throw new Error("retryable timeout");
      return "success";
    };
  ...
  it("escalates after max retries exceeded", async () => {
    await expect(
      withErrorInterception(async () => { throw new Error("unknown error"); }, {
        sourceLayer: "tool",
