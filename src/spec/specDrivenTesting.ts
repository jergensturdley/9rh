...
function classifyStatement(text: string): RequirementKind {
  const lower = text.toLowerCase();

  if (/\b(bug|crashes?|regression|broken)\b/u.test(lower) || /\b(currently\s+fails?|fails?\s+when)\b/u.test(lower)) return "bug";
  if (/^(edge case|edge-case|when|if)\b/u.test(lower)) return "edge";
  ...
  if (/\b(must|only|never|always|without|preserve|constraint)\b/u.test(lower)) return "constraint";
  if (/\b(unclear|maybe|if possible|somehow|better|improve|fast|nice|good)\b/u.test(lower)) return "ambiguous";
  if (startsWithAny(lower, IMPLEMENTATION_VERBS)) return "functional";
  return "functional";
}

function isFailurePathRequirement(text: string): boolean {
  return /^(reject|prevent|block|deny|disallow|fail|validate\s+missing|validate\s+invalid)\b/u.test(text.toLowerCase());
}

function makeRequirement(index: number, kind: RequirementKind, text: string): RequirementStatement {
  return {
    id: `R${index + 1}`,
    kind,
    text,
...
export function parseTaskSpecification(task: string): ParsedSpecification {
  const requirements = splitStatements(task).map((statement, index) =>
    makeRequirement(index, classifyStatement(statement), statement),
  );
...
function testTitle(prefix: string, text: string): string {
...
function addTest(
  ...
  prefix: string,
  expectedInitialState: "unknown" | "failing" = "unknown",
): void {
  const duplicate = tests.some(
    (test) => test.type === type && test.path === path && test.intent === requirement.text,
  );
  if (duplicate) return;
  tests.push({
    id: `T${tests.length + 1}`,
...
export function synthesizeTestPlan(spec: ParsedSpecification): SynthesizedTestPlan {
...
function renderRequirements(label: string, requirements: RequirementStatement[]): string {
  return `### ${label}\n${renderList(requirements.map((requirement) => `${requirement.id}: ${requirement.text}`))}`;
}

export function formatSpecDrivenPrompt(task: string): string {
...
export function shouldUseSpecDrivenTesting(task: string): boolean {
  const lower = task.trim().toLowerCase();
  if (lower.length === 0) return false;
  if (startsWithAny(lower, QUESTION_PREFIXES)) return false;
