...
describe('splash module', () => {
  ...
  describe('shouldShowSplash', () => {
    ...
    afterEach(() => {
      if (envBackup.CI === undefined) delete process.env.CI
      else process.env.CI = envBackup.CI
  ...
  describe('shouldAnimateSplash', () => {
    const envBackup2: Record<string, string | undefined> = {}
    ...
    afterEach(() => {
      if (envBackup2.CI === undefined) delete process.env.CI
      else process.env.CI = envBackup2.CI
      if (envBackup2.NO_COLOR === undefined) delete process.env.NO_COLOR
      else process.env.NO_COLOR = envBackup2.NO_COLOR
    })
