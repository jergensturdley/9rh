...
const SEVERITY_RANK: Record<SemanticSeverity, number> = { info: 0, low: 1, medium: 2, high: 3, critical: 4 };
...
function collectFunctions(path: string, code: string): Map<string, FunctionInfo> {
  ...
  const visit = (node: ts.Node) => {
    ...
    if (name) {
      ...
      const bodyNode = "body" in node ? (node as { body?: ts.Node }).body : undefined;
      const body = bodyNode?.getText(source) ?? text;
      const params = "parameters" in node ? (node as { parameters: ts.NodeArray<ts.ParameterDeclaration> }).parameters.map((p) => p.getText(source)).join(", ") : "";
      const returnType = "type" in node && (node as { type?: ts.TypeNode }).type ? `: ${(node as { type?: ts.TypeNode }).type?.getText(source)}` : "";
      functions.set(name, {
        name,
...
function changedLine(after: string, needle: string): number | undefined {
  const lines = after.split(/\r?\n/u);
  const idx = lines.findIndex((line) => line.includes(needle));
  return idx >= 0 ? idx + 1 : undefined;
}

function addChange(changes: SemanticChange[], change: Omit<SemanticChange, "id">): void {
  changes.push({ id: `sem-${changes.length + 1}`, ...change });
}

function analyzeFile(snapshot: FileSnapshot): SemanticChange[] {
...
function unifiedDiffForFile(snapshot: FileSnapshot): string {
...
export function createPlainDiff(files: FileSnapshot[]): string {
  return files.map(unifiedDiffForFile).join("\n");
}

function assessIntent(task: string, files: FileSnapshot[], changes: SemanticChange[]): IntentRiskAssessment {
  ...
  if (/format|typo|comment|docs?/iu.test(task) && nonFormatting.length > 0) mismatches.push("Task appears formatting/docs-only but implementation changed executable behavior.");
  if (/auth|login|permission|security/iu.test(task) && !changes.some((c) => c.behavior === "security" || c.file.toLowerCase().includes("auth"))) mismatches.push("Task mentions auth/security but no security-sensitive semantic change was detected.");
  if (/test/iu.test(task) && !modified.some((f) => /test|spec|__tests__/iu.test(f))) mismatches.push("Task requests tests but no test files were modified.");
  if (scopeJustificationsRequired.length > 0) mismatches.push("Implementation touches files outside the explicit task/module hints and needs justification.");

  const riskFactors = new Set<string>();
  for (const change of changes) {
    if (change.severity === "critical" || change.severity === "high") riskFactors.add(change.summary);
    if (change.behavior === "data_access") riskFactors.add("Data access behavior changed.");
    if (change.behavior === "security") riskFactors.add("Security-sensitive behavior changed.");
    if (change.behavior === "validation") riskFactors.add("Validation behavior changed.");
  }
  ...
  const riskLevel: SemanticSeverity = riskScore >= 80 ? "critical" : riskScore >= 55 ? "high" : riskScore >= 30 ? "medium" : riskScore > 0 ? "low" : "info";
  const downstreamEffects = changes.flatMap((change) => {
    if (change.behavior === "signature") return [`Callers of ${change.evidence} may need updates.`];
    if (change.behavior === "validation") return [`Inputs reaching ${change.file}:${change.line ?? "?"} may now be accepted/rejected differently.`];
    if (change.behavior === "data_access") return [`Persistence/query behavior may affect stored data or query load.`];
    if (change.behavior === "side_effect") return [`Runtime side effects may affect filesystem, network, logs, or external services.`];
    return [];
  });
...
export function createSemanticReview(task: string, files: FileSnapshot[]): SemanticReview {
...
export function filterSemanticChanges(changes: SemanticChange[], filter: SemanticReviewFilter): SemanticChange[] {
  return changes.filter((change) => {
    if (filter.severity && SEVERITY_RANK[change.severity] < SEVERITY_RANK[filter.severity]) return false;
    if (filter.module && !change.file.includes(filter.module)) return false;
    if (filter.behavior && change.behavior !== filter.behavior) return false;
    return true;
  });
}

export function formatSemanticReview(review: SemanticReview, filter: SemanticReviewFilter = {}): string {
