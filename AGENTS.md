...
- **OpenAI-compatible** at `/v1/*` — used by the agent for completions
- **Native REST** at `/api/*` — used by slash commands (`/status`, `/providers`, `/combos`, `/keys`)
...
`SessionState` is passed by **reference** — mutations from `/switch <model>` and `/dir <path>` persist for all subsequent agent runs and slash commands in the same REPL session.
