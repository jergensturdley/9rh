...
function replaceImageDataUrls(input) {
    const notices = [];
    let imageCount = 0;
    let text = input.replace(MARKDOWN_IMAGE_RE, (_match, alt, url) => {
        const replaced = replaceImageDataUrls(url);
        imageCount += 1;
        notices.push(...replaced.notices);
        return `![${alt || "pasted image"}](${replaced.text})`;
    });
    text = text.replace(DATA_IMAGE_RE, (_match, kind, base64) => {
        imageCount += 1;
        const compact = base64.replace(/\s+/g, "");
...
function stripTerminalNoise(line) {
    return line
        .replace(/\x1b\[[0-9;?]*[ -/]*[@-~]/gu, "")
        .replace(/[╭╮╰╯│┃]/gu, " ")
        .replace(/[─━]{4,}/gu, " ")
...
function compressLargeText(input, opts) {
    const lines = input.split(/\r?\n/u);
    const charCount = input.length;
    const lineCount = lines.length;
    const terminalTraceback = compressTerminalTraceback(input, opts);
    if (terminalTraceback)
        return terminalTraceback;
...
//# sourceMappingURL=inputCompression.js.map