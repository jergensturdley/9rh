...
async function withConfigDir(fn) {
    ...
    try {
        ...
        if (previous === undefined)
            delete process.env.NINE_RH_CONFIG_DIR;
        else
            process.env.NINE_RH_CONFIG_DIR = previous;
...
describe("executeSlashCommand native API auth", () => {
    ...
    it("uses 9router CLI-token auth for /combos", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://127.0.0.1:20128/api/combos", expect.objectContaining({ headers: expect.objectContaining({ "x-9r-cli-token": expect.any(String) }) }));
    });
    it.each([
...
describe("model command helpers", () => {
    ...
    it("fetches OpenAI-compatible models with the session API key", async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith("http://127.0.0.1:20128/v1/models", expect.objectContaining({ headers: { Authorization: "Bearer model-key" } }));
    });
    ...
    it("/switch changes only the current REPL session model", async () => {
        jest.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
            if (String(url).endsWith("/v1/models"))
                return jsonResponse({ data: [{ id: "kr/next" }] });
            if (String(url).endsWith("/api/providers"))
                return jsonResponse({ connections: [] });
            return jsonResponse({ error: "not found" }, { status: 404 });
        });
    ...
    it("/default-model persists the startup model and updates current session", async () => {
        await withConfigDir(async (dir) => {
            jest.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
                if (String(url).endsWith("/v1/models"))
                    return jsonResponse({ data: [{ id: "kr/persisted" }] });
                if (String(url).endsWith("/api/providers"))
                    return jsonResponse({ connections: [] });
                return jsonResponse({ error: "not found" }, { status: 404 });
            });
    ...
    it("caches model and provider config for repeated slash menu lookups", async () => {
        ...
        const fetchMock = jest.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
            if (String(url).endsWith("/v1/models"))
                return jsonResponse({ data: [{ id: "kr/model-a", owned_by: "kr" }] });
            if (String(url).endsWith("/api/providers"))
                return jsonResponse({ connections: [{ provider: "kiro", name: "Kiro", isActive: true }] });
            return jsonResponse({ error: "not found" }, { status: 404 });
        });
    ...
    it("/router summarizes cached 9router configuration", async () => {
        jest.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
            ...
            if (String(url).endsWith("/api/combos"))
                return jsonResponse({ combos: [{ name: "fast", models: ["kr/model-a"] }] });
            if (String(url).endsWith("/api/keys"))
                return jsonResponse({ keys: [{ id: "key_1", name: "Default" }] });
            return jsonResponse({ error: "not found" }, { status: 404 });
        });
    ...
    it("/refresh clears stale router config cache before refetching", async () => {
        ...
        jest.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
            if (String(url).endsWith("/v1/models"))
                return jsonResponse({ data: [{ id: modelId, owned_by: "kr" }] });
            if (String(url).endsWith("/api/providers"))
                return jsonResponse({ connections: [{ provider: "kiro", isActive: true }] });
            return jsonResponse({ error: "not found" }, { status: 404 });
        });
...
describe("debug-auth handler", () => {
    ...
    async function withMockHome(fn) {
        ...
        await withMockHome(async (home) => {
            ...
            const fetchMock = jest.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
                ...
                if (path === "/api/health")
                    return jsonResponse({ ok: true });
                if (path === "/api/providers")
                    return jsonResponse({ connections: [1, 2] });
                return jsonResponse({ error: "not found" }, { status: 404 });
            });
    ...
    it("falls back to API key when token missing", async () => {
        await withMockHome(async (_home) => {
            const fetchMock = jest.spyOn(globalThis, "fetch").mockImplementation(async (url) => {
                ...
                if (path === "/api/providers")
                    return jsonResponse({ error: "Service Unavailable" }, { status: 503, statusText: "Service Unavailable" });
                return jsonResponse({ error: "not found" }, { status: 404 });
            });
