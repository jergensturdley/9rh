...
import { classifyRisk, isTrivialEdit, requiresSecurityAudit, requiresTestStrategy, } from "../orchestrator/roles.js";
import { canOverride, createConflictLog, recordConflict, resolveConflict, } from "../orchestrator/conflictResolver.js";
import { cachePlan, cacheTestStrategy, createOrchestratorCache, getCachedPlan, getCachedTestStrategy, getCacheStats, hashTask, hashTaskAndFiles, } from "../orchestrator/performanceCache.js";
...
describe("resolveConflict", () => {
    it("escalates security_auditor blocking conflict to human", () => {
        const { resolution } = resolveConflict({ parties: ["security_auditor", "implementer"], description: "critical vuln", severity: "blocking" }, 0, 2);
        expect(resolution).toBe("escalate_human");
    });
    it("escalates security_auditor major conflict to coordinator", () => {
        const { resolution } = resolveConflict({ parties: ["security_auditor", "implementer"], description: "medium concern", severity: "major" }, 0, 2);
        expect(resolution).toBe("escalate_coordinator");
    });
    it("sends implementer to revise within maxRevisions", () => {
        const { resolution } = resolveConflict({ parties: ["reviewer", "implementer"], description: "code quality issues", severity: "major" }, 0, 2);
        expect(resolution).toBe("implementer_revises");
    });
    it("escalates to human when maxRevisions exhausted", () => {
        const { resolution } = resolveConflict({ parties: ["reviewer", "implementer"], description: "ongoing disagreement", severity: "major" }, 2, 2);
        expect(resolution).toBe("escalate_human");
    });
    it("escalates test_strategist conflict to coordinator", () => {
        const { resolution } = resolveConflict({ parties: ["test_strategist", "implementer"], description: "coverage gap", severity: "major" }, 0, 2);
        expect(resolution).toBe("escalate_coordinator");
    });
});
...
describe("hashing", () => {
    ...
    it("hashTaskAndFiles is deterministic and file-order-independent", () => {
        expect(hashTaskAndFiles("task", ["b.ts", "a.ts"])).toBe(hashTaskAndFiles("task", ["a.ts", "b.ts"]));
    });
    it("hashTaskAndFiles differs when files differ", () => {
        expect(hashTaskAndFiles("task", ["a.ts"])).not.toBe(hashTaskAndFiles("task", ["b.ts"]));
    });
});
...
function makeOrchestrator(roleInvoker, opts = {}) {
...
describe("Orchestrator — happy path (low risk, no security, reviewer approves)", () => {
    it("emits task_complete and returns completed status", async () => {
        const events = [];
        const invoker = jest.fn(async (role) => {
            if (role === "architect")
                return lowRiskPlan;
            if (role === "implementer")
                return implementationOk;
            if (role === "reviewer")
        ...
        const roles = events.filter((e) => e.type === "role_complete").map((e) => e.role);
...
describe("Orchestrator — trivial edit skips reviews", () => {
    it("skips reviewer, security_auditor, test_strategist for trivial edits", async () => {
        ...
        expect(result.status).toBe("completed");
        const skipped = events.filter((e) => e.type === "role_skip").map((e) => e.role);
...
describe("Orchestrator — cache_hit events", () => {
    it("emits cache_hit on second call with same task", async () => {
        ...
        events.length = 0;
        await o.orchestrate("Add a helper function to src/foo.ts");
        expect(events.some((e) => e.type === "cache_hit" && e.role === "architect")).toBe(true);
    });
});
