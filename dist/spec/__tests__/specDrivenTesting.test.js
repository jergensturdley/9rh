...
import { formatSpecDrivenPrompt, parseTaskSpecification, shouldUseSpecDrivenTesting, synthesizeTestPlan, } from "../specDrivenTesting.js";
...
describe("parseTaskSpecification", () => {
    ...
    it("treats validation errors as failure-path behavior, not automatic bug reports", () => {
        ...
        expect(plan.tests).toEqual(expect.arrayContaining([
    ...
    it("maps reject and prevent requirements to failure or guard-path tests", () => {
        ...
        const plan = synthesizeTestPlan(spec);
        const negativeTests = plan.tests.filter((test) => test.intent.includes("Reject empty carts") || test.intent.includes("Prevent duplicate submissions"));
...
//# sourceMappingURL=specDrivenTesting.test.js.map