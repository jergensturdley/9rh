...
const realWorkDirCache = new Map();
const sandboxPathCache = new Map();
...
export const TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        function: {
            ...
            description: "Search the local CodeGraph semantic index for symbols by name. Prefer this over grep for symbol discovery when .codegraph/ exists.",
            parameters: {
                type: "object",
                properties: {
    ...
    },
    {
        ...
        function: {
            ...
            description: "Build task-focused context from the local CodeGraph index, including entry points, related symbols, and optional code snippets. Use before broad file exploration.",
            parameters: {
                type: "object",
                properties: {
    ...
    },
    {
        ...
        function: {
            ...
            description: "Show indexed project file structure from CodeGraph. Use instead of recursive ls when .codegraph/ exists.",
            parameters: {
                type: "object",
                properties: {
    ...
    },
    {
        ...
        function: {
            ...
            description: "Find test files affected by changed source files using CodeGraph dependency traversal.",
            parameters: {
                type: "object",
                properties: {
...
async function runCodegraph(args, workDir) {
    try {
...
function addNumberFlag(args, flag, value) {
...
async function toolCodegraphSearch(args, workDir) {
    const query = typeof args.query === "string" ? args.query.trim() : "";
    if (!query)
        return { output: "", error: "codegraph_search requires query" };
...
async function toolCodegraphContext(args, workDir) {
    const task = typeof args.task === "string" ? args.task.trim() : "";
    if (!task)
        return { output: "", error: "codegraph_context requires task" };
...
async function toolCodegraphFiles(args, workDir) {
    ...
    if (args.no_metadata)
        cgArgs.push("--no-metadata");
    if (args.json)
        cgArgs.push("--json");
    return runCodegraph(cgArgs, workDir);
}
async function toolCodegraphAffected(args, workDir) {
    const files = Array.isArray(args.files) ? args.files.filter((f) => typeof f === "string" && f.trim() !== "") : [];
    if (files.length === 0)
        return { output: "", error: "codegraph_affected requires files" };
    for (const file of files)
        await sandboxPath(file, workDir);
...
async function toolCodegraphStatus(args, workDir) {
