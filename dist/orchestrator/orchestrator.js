...
import { ROLE_DEFINITIONS, classifyRisk, requiresSecurityAudit, requiresTestStrategy, isTrivialEdit, } from "./roles.js";
import { createTaskState, createProjectMemory, taskStateToContext, } from "./taskState.js";
import { createConflictLog, resolveConflict, recordConflict, } from "./conflictResolver.js";
import { createOrchestratorCache, hashTask, hashTaskAndFiles, getCachedPlan, cachePlan, getCachedTestStrategy, cacheTestStrategy, getCacheStats, } from "./performanceCache.js";
...
function parseRoleOutput(output, fallback) {
    try {
        return JSON.parse(stripMarkdownFences(output));
    }
    catch {
        return fallback;
    }
...
export class Orchestrator {
    ...
    constructor(config) {
        ...
        if (config.roleInvoker) {
            ...
            this.invokeRole = async (role, prompt) => {
    ...
    async runArchitect(state) {
        ...
        const output = await this.invokeRole("architect", `Analyze this task and produce a structured implementation plan:\n\n${taskStateToContext(state, "architect")}`);
        this.emit({ type: "role_complete", role: "architect", taskId: state.id, result: output });
        const plan = parseRoleOutput(output, {
    ...
    async runReviewer(state) {
        this.emit({ type: "role_start", role: "reviewer", taskId: state.id });
        const output = await this.invokeRole("reviewer", `Review this implementation and provide a detailed assessment:\n\n${taskStateToContext(state, "reviewer")}`);
    ...
    async runSecurityAuditor(state) {
        this.emit({ type: "role_start", role: "security_auditor", taskId: state.id });
        const output = await this.invokeRole("security_auditor", `Perform a security audit of these high-risk changes:\n\n${taskStateToContext(state, "security_auditor")}`);
    ...
    async runTestStrategist(state) {
        const cacheKey = hashTaskAndFiles(state.originalTask, state.implementationResult?.filesModified ?? []);
        ...
        const output = await this.invokeRole("test_strategist", `Define and assess the test strategy for this implementation:\n\n${taskStateToContext(state, "test_strategist")}`);
    ...
    buildResult(state, conflictLog, error) {
        return {
            taskId: state.id,
            status: state.status,
        ...
        try {
            ...
            if (state.architectPlan.requiresSecurityAudit && !trivial) {
                ...
                if (state.securityAuditResult.clearance === "rejected") {
                    ...
                    const { resolution, justification } = resolveConflict({ parties: conflict.parties, description: conflict.description, severity: conflict.severity }, state.revisionCount, state.maxRevisions);
            ...
            while (!trivial && !reviewerApproved && state.revisionCount <= state.maxRevisions) {
                ...
                if (state.reviewResult.decision === "approved") {
                    ...
                    const severity = state.reviewResult.decision === "rejected" ? "blocking" : "major";
                    const conflict = recordConflict(conflictLog, {
